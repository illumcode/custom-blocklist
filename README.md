...
| **google_full** | полный официальный список сетей Google (`goog.json`), нужен для YouTube: видео раздаётся с огромного числа edge-узлов, один снимок DNS-резолва их все не поймает |
| **extra_cidrs** | подсети, зашитые прямо в конфиг руками, для случаев, когда часть трафика прыгает между блоками так, что резолв домена не успевает это поймать (пример: Gemini и `waa-pa.clients6.google.com`) |
